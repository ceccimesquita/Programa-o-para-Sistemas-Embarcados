...
void Delay(unsigned int mSec){
	...
	HWREG(SOC_DMTIMER_7_REGS + DMTIMER_TCRR) = countVal; //timer7 + tcrr = 3Ch
	
	flag_timer = false;

    /* Enable the DMTimer interrupts */
	HWREG(SOC_DMTIMER_7_REGS + DMTIMER_IRQENABLE_SET) = 0x2;  //timer7 + irqstatus+set = 2Ch
	...
	HWREG(SOC_DMTIMER_7_REGS + DMTIMER_IRQENABLE_CLR) = 0x2; //timer7 + irqstatus+clr = 30h
}
