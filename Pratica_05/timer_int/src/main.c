...
#define DMTimerWaitForWrite(reg)   \
            if(HWREG(DMTIMER_TSICR) & 0x4)\ // dmtimer7 = 0x44E0_5000 + tsicr = 0x54
...
void timerEnable(){
    ...
    /* Start the timer */
    HWREG(DMTIMER_TCLR) |= 0x1;  //timer7 + tclr = 38h
...
void timerDisable(){
    ...
    /* Stop the timer */
    HWREG(DMTIMER_TCLR) &= ~(0x1); //timer 7 + tclr = 38h
}/* -----  end of function timerEnable  ----- */
...
void delay(unsigned int mSec){
	...
	HWREG(DMTIMER_TCRR) = countVal; //timer7 + tcrr = 3Ch
	
	flag_timer = false;

    /* Enable the DMTimer interrupts */
	HWREG(DMTIMER_IRQENABLE_SET) = 0x2;  //timer7 + irqstatus+set = 2Ch
	...
	HWREG(DMTIMER_IRQENABLE_CLR) = 0x2; //timer7 + irqstatus+clr = 30h
}
...
void timerSetup(void){
    ...
    HWREG(CM_PER_TIMER7_CLKCTRL) |= 0x2; //cmper = 0x44E0_0000 + timer7 = 7Ch

	/*  Check clock enable for DMTIMER7 TRM 8.1.12.1.25 */    
    while((HWREG(CM_PER_TIMER7_CLKCTRL) & 0x3) != 0x2); //espera ate ativar o clok
