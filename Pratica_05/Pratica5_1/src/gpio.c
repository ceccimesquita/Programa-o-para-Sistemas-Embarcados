    		...
    		unsigned int setting = (1<<18) | (0x2<<0);   //enable functional clock & enable module, TRM 8.1.12.1.29
      		switch(mod){
        		case GPIO0:
            		...
            		break;
         		case GPIO1:
            			ckmSetCLKModuleRegister(CKM_PER, 0xAC , setting);
            			while((ckmGetCLKModuleRegister(CKM_PER, 0xAC) & (0x3<<16)) != 0)
            		break;
         		case GPIO2:
...
int gpio_interrupt_config(gpioMod gpio, ucPinNumber pin, int type){
    switch (gpio)
    {
    case GPIO0:
        if(type == 0){
